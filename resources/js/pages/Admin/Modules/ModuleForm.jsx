...
export default function ModuleForm({ data, setData, errors, processing, onSubmit, isEdit = false }) {
        ...
        <form onSubmit={onSubmit}>
            <div className="grid grid-cols-1 gap-x-6 gap-y-6">
                <div className="sm:col-span-full">
                    <Label htmlFor="nom" className="text-[var(--foreground)]">
                        {translations?.module_form_name_label || 'Nom du Module'} *
                    </Label>
                    <div className="mt-1">
                        <Input
                            type="text"
            ...
            <div className="mt-8 flex items-center justify-end gap-x-4 border-t border-[var(--border)] pt-6">
                <Button variant="outline" type="button" asChild>
                    <Link href={route('admin.modules.index')}>{translations?.cancel_button || 'Annuler'}</Link>
                </Button>
                <Button
                    type="submit"
                    disabled={processing}

...
import ConfirmationModal from '@/components/Common/ConfirmationModal'; // Re-use the modal

const defaultPageSize = 15;
const defaultPageIndex = 0;

export default function Index({ modules: modulesPagination, filters }) { // Changed prop name
    ...
    useEffect(() => {
        if (
            pagination.pageIndex !== modulesPagination.current_page - 1 ||
            pagination.pageSize !== modulesPagination.per_page
        ) {
            router.get(
    ...
    const openDeleteModal = (moduleItem) => { // Renamed parameter
    ...
    const confirmDelete = () => {
        if (itemToDelete) {
            router.delete(route('admin.modules.destroy', { module: itemToDelete.id }), { // Changed route name
                preserveScroll: true,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <ConfirmationModal
                ...
                message={
                    itemToDelete
                        ? (translations?.module_delete_confirmation || 'Are you sure you want to delete the module "{name}"?').replace('{name}', itemToDelete.nom)
